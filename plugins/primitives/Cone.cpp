...
class Cone : public IPrimitive {
    public:
        ...
        Cone(const libconfig::Setting &config)
        {
        ...
        double getIntersection(const Ray &ray) const override
        {
            ...
            double tanAngle = _radius / _height;
            double tanAngleSq = tanAngle * tanAngle;

            double a = dPerp.dot(dPerp) - tanAngleSq * dDotA * dDotA;
            double b = 2.0 * (ocPerp.dot(dPerp) - tanAngleSq * ocDotA * dDotA);
            double c = ocPerp.dot(ocPerp) - tanAngleSq * ocDotA * ocDotA;
            ...
            if (std::abs(a) > 1e-12) {
                ...
                if (disc >= 0.0) {
                    ...
                    for (double t : {t0, t1}) {
                        ...
                        if (axisPos >= -_height - 1e-8 && axisPos <= 0 + 1e-8) {
        ...
        Math::Vector3D getNormal(const Math::Vector3D &point) const override
        {
            ...
            if (std::abs(axisPos) < EPSN)
                return axis;

            Math::Vector3D radial = pc - axis * axisPos;
            Math::Vector3D n = radial + axis * (_radius / _height);
            return n.normalize();
        }
        ...
        AABB getAABB() const override
        {
            ...
            Math::Vector3D base(
                _center.x - ax.x * _height,
                _center.y - ax.y * _height,
                _center.z - ax.z * _height
            );
            ...
            double minX = std::min((double)_center.x, base.x - ex);
            double minY = std::min((double)_center.y, base.y - ey);
            double minZ = std::min((double)_center.z, base.z - ez);
            double maxX = std::max((double)_center.x, base.x + ex);
            double maxY = std::max((double)_center.y, base.y + ey);
            double maxZ = std::max((double)_center.z, base.z + ez);
